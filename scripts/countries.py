"""国家/地区元数据：ISO 3166-1 alpha-2 代码、英文名、中文名。

约束（详见 DEVELOPMENT.md「数据源与国家覆盖」）：
- 仅收录 ISO 3166-1 alpha-2 标准代码；
- 额外收录 RIR 数据中出现的非 ISO 特殊码：XK / EU / AP / ZZ / A1 / A2；
- 中文名采用常用简称，便于规则使用者辨识。
"""

from __future__ import annotations

# code -> (english_name, chinese_name)
COUNTRIES: dict[str, tuple[str, str]] = {
    ...
    "BO": ("Bolivia", "玻利维亚"),
    "BQ": ("Bonaire, Sint Eustatius and Saba", "博奈尔、圣尤斯特歇斯和萨巴"),
    "BR": ("Brazil", "巴西"),
    "BS": ("Bahamas", "巴哈马"),
    "BT": ("Bhutan", "不丹"),
    ...
    "GR": ("Greece", "希腊"),
    "GS": ("South Georgia and the South Sandwich Islands", "南乔治亚和南桑威奇群岛"),
    "GT": ("Guatemala", "危地马拉"),
    "GU": ("Guam", "关岛"),
    ...
    "SE": ("Sweden", "瑞典"),
    "SG": ("Singapore", "新加坡"),
    "SH": ("Saint Helena, Ascension and Tristan da Cunha", "圣赫勒拿"),
    "SI": ("Slovenia", "斯洛文尼亚"),
    ...
    "SV": ("El Salvador", "萨尔瓦多"),
    "SX": ("Sint Maarten (Dutch part)", "荷属圣马丁"),
    "SY": ("Syria", "叙利亚"),
    "SZ": ("Eswatini", "斯威士兰"),
...
# 非 ISO 特殊码（RIR / 业界通用），仍提供命名以便显示。
EXTRA: dict[str, tuple[str, str]] = {
    "XK": ("Kosovo", "科索沃"),
    "EU": ("European Union", "欧盟"),
    "AP": ("Asia Pacific", "亚太地区"),
    "ZZ": ("Unknown / Unassigned", "未知 / 未分配"),
    "A1": ("Anonymous Proxy", "匿名代理"),
    "A2": ("Satellite Provider", "卫星服务商"),
    "GLOBAL": ("Global", "全球"),
}

ALL: dict[str, tuple[str, str]] = {**COUNTRIES, **EXTRA}


def name_en(code: str) -> str:
    return ALL.get(code.upper(), (code, code))[0]


def name_zh(code: str) -> str:
    return ALL.get(code.upper(), (code, code))[1]


def display(code: str) -> str:
    en, zh = ALL.get(code.upper(), (code, code))
    return f"{code} - {zh}（{en}）"
